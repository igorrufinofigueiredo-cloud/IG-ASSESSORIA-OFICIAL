...
Os dois vídeos (a logo sendo desenhada no topo e o vídeo da seção de marca) também estão embutidos no `index.html`.
...
2. Se já existir um `index.html` lá, apagar primeiro: clicar no arquivo → ícone de lixeira → **Commit changes**.
   Fazer o mesmo com `styles.css`, `script.js`, `hero-ig-desenho.mp4` e `hero-ig-poster.jpg`, se existirem: não são mais usados.
